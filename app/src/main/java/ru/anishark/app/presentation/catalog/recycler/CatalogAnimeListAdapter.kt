...
class CatalogAnimeListAdapter(
    val data: List<AnimeModelForCatalog> =
        MutableList(10) {
            AnimeModelForCatalog(
                name = "Берсерк ${it + 1}",
                description =
                    "Боль, кровь и слёзы. На престол восходит новый правитель. " +
                        "Его слуги, подручные демоны безнаказанно творят бесчинства в городе. " +
                        "Все меняется когда в город, скрытый темнотой ночи, попадает тяжёлый воин. " +
                        "С разнообразным вооружением для самых непредвиденных моментов, " +
                        "в броне, тело его все покрыто шрамами – черный мечник. " +
                        "Его меч настолько огромен, " +
                        "что может сравниться с его лютой злобой к королю и демонам.",
                episodesCount = 24,
            )
        },
        ...
        fun bind(model: AnimeModelForCatalog) {
            ...
            binding.cardIv.setImageDrawable(
                AppCompatResources.getDrawable(
                    ...
                    R.drawable.default_anime_catalog_image,
                ),
            )
        }
    }

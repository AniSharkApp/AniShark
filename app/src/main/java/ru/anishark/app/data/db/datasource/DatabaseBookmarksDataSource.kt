...
class DatabaseBookmarksDataSource @Inject constructor(
    ...
    fun getAllBookmarks(): Observable<List<BookmarkModel>> = dao.getAll().map { it.map { entity -> entity.toModel() } }

    fun getBookmark(animeId: Int): Observable<BookmarkModel> = dao.getBookmark(animeId).map { it.toModel() }

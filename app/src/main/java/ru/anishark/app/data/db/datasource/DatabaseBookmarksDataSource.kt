...
class DatabaseBookmarksDataSource @Inject constructor(
    ...
    override fun getAllBookmarks(): Single<List<BookmarkModel>> = dao.getAll().map { it.map { entity -> entity.toModel() } }

    override fun getBookmark(animeId: Int): Single<BookmarkModel> = dao.getBookmark(animeId).map { it.toModel() }
